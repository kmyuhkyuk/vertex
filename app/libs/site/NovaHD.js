...
class Site {
  constructor () {
    this.name = 'NovaHD';
    this.downloadLink = 'https://pt.novahd.top/download.php?id={ID}';
    this.url = 'https://pt.novahd.top/';
  };
  ...
  async searchTorrent (keyword) {
    const torrentList = [];
    const document = await this._getDocument(`${this.index}torrents.php?notnewword=1&incldead=1&spstate=0&inclbookmarked=0&search=${encodeURIComponent(keyword)}&search_area=${keyword.match(/tt\d+/) ? 4 : 0}&search_mode=0&tag=`);
